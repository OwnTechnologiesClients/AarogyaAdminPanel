...
import { Search, Trash2, Edit, Eye, Filter, Microscope } from "lucide-react"
import Link from "next/link"
import Swal from "sweetalert2"
import EquipmentApi from "@/lib/api/equipmentApi"
import { getApiErrorMessage } from "@/lib/utils"

export default function MedicalEquipmentsList() {
  ...
  const handleToggleStatus = async (id, currentStatus, name) => {
    ...
    try {
      await EquipmentApi.update(id, { isActive: newStatus })
      ...
      await Swal.fire({
        ...
        text: getApiErrorMessage(error, "Failed to update equipment status. Please try again."),
        icon: "error",
        timer: 2000,
      })
    }
  ...
  const handleDelete = async (id, name) => {
    const result = await Swal.fire({
      title: "Are you sure?",
      text: `You are about to delete equipment "${name}". This action cannot be undone!`,
      icon: "warning",
    ...
    if (result.isConfirmed) {
      try {
        ...
        await Swal.fire({
          ...
          text: getApiErrorMessage(error, "Failed to delete equipment. Please try again."),
          icon: "error",
          confirmButtonText: "OK",
        })
      }
    ...
    <Layout>
      <div className="space-y-6">
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-3xl font-bold text-gray-900 mb-2">Medical Equipments</h1>
            <p className="text-gray-700 text-lg">Manage master list of advanced medical equipments</p>
          </div>
          <Link href="/medical-equipments/add">
            ...
            <div className="flex justify-between items-center">
              ...
              {searchTerm && (
                <div className="text-sm text-gray-800 font-semibold bg-blue-100 px-4 py-2 rounded-lg">
                  Found <span className="text-blue-700 font-bold">{totalItems}</span> equipment
                  {totalItems !== 1 ? "s" : ""}
                  ...
                  <tr>
                    <td colSpan="4" className="px-4 py-8 text-center text-gray-500">
                      <div className="flex flex-col items-center space-y-2">
                        ...
                        <span className="text-lg font-medium">No equipments found</span>
                        <span className="text-sm">
                          {searchTerm ? `No equipments match "${searchTerm}"` : "Start by adding your first equipment"}
                        </span>
                      </div>
                    </td>
                  </tr>
                    ...
                    <tr key={item._id} className="hover:bg-blue-50 transition-colors">
                      ...
                      <td className="px-4 py-3 whitespace-nowrap text-sm font-medium">
                        <div className="flex items-center space-x-2">
                          ...
                          <Link
                            href={`/medical-equipments/edit/${item._id}`}
                          ...
                          <Link
                            href={`/medical-equipments/view/${item._id}`}
